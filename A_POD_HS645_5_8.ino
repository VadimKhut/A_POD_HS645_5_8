...
//
//  Hardware: Arduino Mega, SSC32 V2, PS2
//
//--------------------------------------------------------------------
...
void setup(){
  ...
  pinMode(SS_p, OUTPUT);
  digitalWrite(SS_p, HIGH);           // set pin _SS HIGH state
  pinMode(EOM_p, INPUT);
  pinMode(JUMPER_p, INPUT);
  pinMode(JUMPER1_p, INPUT);
  pinMode(Eyes_p, OUTPUT);
  pinMode(Led_RED_p, OUTPUT);
  pinMode(Led_YELL_p, OUTPUT);
  pinMode(Led_GREEN_p, OUTPUT);
  
  
  //ISD_MuteOn();                     // set pin _MUTE LOW state
  
  digitalWrite(Eyes_p, LOW);          // set Eyes in OFF state
  digitalWrite(Led_RED_p, HIGH);      // set pin Led_RED_p in OFF state
  digitalWrite(Led_YELL_p, HIGH);     // set pin Led_YELL_p in OFF state
  digitalWrite(Led_GREEN_p, HIGH);    // set pin Led_GREEN_p in OFF state
  digitalWrite(EOM_p, HIGH);          // enable an internal 20K pullup resistor  for input pin
...
//******************************************************************
#ifdef SaveToEEPROM
  DBGSerial.println(F("Save to EEPROM"));  
  ReadPort();                                // Confirmation save to EEPROM    

  for (int i=0; i < sizeof(AdrRec); i++) {
    EEPROM.write(i, pgm_read_byte(&AdrRec[i]));
  }
#endif  

#ifdef DEBUG                                // read Adr Record From EEPROM
  byte b;
  b = sizeof(AdrRec);
  DBGSerial.println(b, DEC);
  for (int i=0; i < sizeof(AdrRec); i++) {
    if(i % 16 == 0) DBGSerial.println();
    b = EEPROM.read(i);
...
// Set Jumper to pin 23 and ground for disable sound from ISD4004 
  if (digitalRead(JUMPER1_p) == 0){
    ISD_PowerDown();                            // ISD Power Down
    F_SoundEnable = false;
  #ifdef DEBUG
    DBGSerial.println(F("ISD Power Down"));
  #endif  
  }
  else{
    ISD_PowerUp();                              // ISD Power Up
    F_SoundEnable = true;
  #ifdef DEBUG
    DBGSerial.println(F("ISD Power UP"));
  #endif    
  }  

  delay(100);                                  // delay ISD PowerUp

// Set Jumper to pin 22 and ground for listening all records from ISD4004 
  while(!digitalRead(JUMPER_p)){
   #ifdef DEBUG
     DBGSerial.println(F("Listening all records"));
   #endif          
    for (int i=0; i < sizeof(AdrRec)/2; i++) {
   #ifdef DEBUG
      DBGSerial.println(i, DEC);
      //ReadPort();                                // Confirmation
   #endif          
      if(i == 0){
        ISD_PlayFromADR(i, IGNORE_EOM);
      }
      else{  
      ISD_PlayFromADR(i, WAIT_EOM_PREV);
      }
      MSound( 1, 50, 2000);             // [50\4000]
      delay(1000);
      if(digitalRead(JUMPER_p)){
        ISD_Stop();
     #ifdef DEBUG
        DBGSerial.println(F("ISD Stop"));
        delay(500);
     #endif          
        break;
      }  
    }
    if(digitalRead(JUMPER_p)) break;
  }
  ...
  if (F_SoundEnable){
    ISD_PlayFromADR(38, WAIT_EOM_AFTER);   // 38 приветствие.wav
  }
...
void loop(void){
  ...
  #ifdef ApodCode
    if (LegIndex == cRR){
      RR_RealPosY = (LegPosY[LegIndex] + g_InControlState.BodyPos.y - BodyFKPosY + GaitPosY[LegIndex]);
      RR_RealPosX = (LegPosX[LegIndex] - g_InControlState.BodyPos.x + BodyFKPosX - GaitPosX[LegIndex]);
    }   
  #endif   
  }
  ...
  #ifdef ApodCode
    if (LegIndex == cLR){
      LR_RealPosY = (LegPosY[LegIndex] + g_InControlState.BodyPos.y - BodyFKPosY + GaitPosY[LegIndex]);
      LR_RealPosX = (LegPosX[LegIndex] + g_InControlState.BodyPos.x - BodyFKPosX + GaitPosX[LegIndex]);
    }   
  #endif   
  }
...
void StartUpdateServos(void) {        
  ...
  for (LegIndex = 0; LegIndex <= 4; LegIndex++) {
    g_ServoDriver.OutputServoForHeadAbdomen(LegIndex, HeadAbdomenAngle1[LegIndex]);  // ###Vad1
  }
  
  g_ServoDriver.OutputServoForMand();     // ###Vad1
}
...
//[Abdomen Control]
//--------------------------------------------------------------------
  void AbdomenControl(void) {
    AbdomPanAngle1 = (RR_RealPosX - LR_RealPosX) * c1DEC / 3;
    AbdomTiltAngle1 = -2 * g_InControlState.BodyRot1.x + 150 - (g_InControlState.BodyPos.y * 5 + (RR_RealPosY + LR_RealPosY - 30) / 4);

#ifdef DEBUG
    if (g_fDebugOutput) {
      if (g_fDebugAbdom) {
        // DEBUG Amdomen Control
        DBGSerial.println(F("AbdomCon:"));
        DBGSerial.print(F("RR_X="));
        DBGSerial.print(RR_RealPosX, DEC);  
        DBGSerial.print(F(" LR_X="));
        DBGSerial.print(LR_RealPosX, DEC);  
        DBGSerial.print(F(" RR_Y="));
        DBGSerial.print(RR_RealPosY, DEC);  
        DBGSerial.print(F(" LR_Y="));
        DBGSerial.print(LR_RealPosY,DEC);  
        DBGSerial.print(F(" AbP="));
        DBGSerial.print(AbdomPanAngle1,DEC);  
        DBGSerial.print(F(" AbT="));
        DBGSerial.print(AbdomTiltAngle1, DEC);  
        DBGSerial.print(F(" BPY="));
        DBGSerial.print(g_InControlState.BodyPos.y, DEC);  
        DBGSerial.print(F(" BRX="));
    ...
    if ((ich = DBGSerial.available())) {
      ...
      if (ich == 0) {
        ...
        switch (szCmdLine[1]){
         ...
         break;
         case '9': 
           g_fDebugMonitorFSR = !g_fDebugMonitorFSR;
         break;
         case 'a': 
         ...
         break;
         case 'c': 
           g_fDebug_HeadAbdomen = !g_fDebug_HeadAbdomen;
         break;
         case 'D': 
...
void ISD_PowerUp(void){
  ...
  //ISD_MuteOn();
  digitalWrite(SS_p, LOW);                // set pin _SS LOW state
...
//--------------------------------------------------------------------
// ISD_PlayFromADR
//ISD_PlayFromADR(byte n_record, byte checkBusy)
// checkBusy: 0 - not check (IGNORE_EOM)
//            1 -  return, if busy (SKEEP_IF_BYSU)
//            2 - wait, EOM prev (WAIT_EOM_PREV)
//            3 - wait EOM after (WAIT_EOM_AFTER)
//            3 - wait EOM prev and after (WAIT_EOM_PREV_AFTER)
//--------------------------------------------------------------------
void ISD_PlayFromADR(byte n_record, byte checkBusy){
  byte ISD_Adr_H; byte ISD_Adr_L;
  
  if(checkBusy == SKEEP_IF_BYSU){               // return, if busy
    if(digitalRead(EOM_p) == 1) return;
  }
  else if(checkBusy == WAIT_EOM_PREV){          // wait, if busy
    while(digitalRead(EOM_p) == 1);
  } 
  else if(checkBusy == WAIT_EOM_PREV_AFTER){    // wait, if busy
    while(digitalRead(EOM_p) == 1);
    checkBusy=WAIT_EOM_AFTER;
  } 
  else if(checkBusy == IGNORE_EOM){             // ignore EOM
    if(digitalRead(EOM_p) == 1){
     ISD_Stop();
     delay(50);
    } 
  } 
  ...
  #ifdef DEBUG
    Serial.print(F("NRec= "));
    DBGSerial.print(n_record, DEC);
  #endif

    n_record = n_record*2;
  #ifdef ReadAdrFromEEPROM
    ISD_Adr_H = EEPROM.read(n_record);
    ISD_Adr_L = EEPROM.read(n_record+1);
  #else
    ISD_Adr_H = pgm_read_byte(&n_record);
    ISD_Adr_L = pgm_read_byte(&n_record+1);
  #endif  

  
  //Set Adr Poinrer ISD --> PLAY
  digitalWrite(SS_p, LOW);                // set pin _SS LOW state
  SPI.transfer(ISD_Adr_L);
  SPI.transfer(ISD_Adr_H);
  SPI.transfer(SETPLAY);
  digitalWrite(SS_p, HIGH);               // set pin _SS HIGH state -> begin operation
  
  //Set bit IAB=1 --> PLAY from current Adr
  digitalWrite(SS_p, LOW);                // set pin _SS LOW state
  SPI.transfer(PLAY);
  digitalWrite(SS_p, HIGH);               // set pin _SS HIGH state
  
  if(checkBusy == WAIT_EOM_AFTER){        // wait, for EOM
    delay(50);
    while(digitalRead(EOM_p) == 1);
  }

  #ifdef DEBUG
    Serial.print(F("  NRec*2= "));
    DBGSerial.print(n_record, DEC);
    Serial.print(F("\tAdr= "));
    DBGSerial.print(ISD_Adr_H, HEX);
    DBGSerial.println(ISD_Adr_L, HEX);
  #endif
}  
...
// ISD_MuteOn/Off
//--------------------------------------------------------------------
void ISD_MuteOn(void){
  pinMode(MUTE_p, OUTPUT);
  digitalWrite(MUTE_p, LOW);               // set pin _MUTE LOW state
}  

void ISD_MuteOff(void){
  pinMode(MUTE_p, INPUT);
}  
...
void PlayBackgroundSound(word T_Play_Every){
  ...
  if (F_SoundEnable){
    ...
    if ((TimerBackgroundSound_t - TimerBackgroundSound) > T_Play_Every){     // play Background sound every TPlay_Every
      ISD_PlayFromADR(24, SKEEP_IF_BYSU);
      TimerBackgroundSound = millis();                                       // reset TimerBackgroundSound
    }
  }  
}
...
byte GetnSeqCycDelay(short CountSeqAtt){
  ...
  if(CountSeqAtt == 0){
    ...
    CountSeqAtt = (CountSeqAtt - 1) * 12;  
    nSeqCycDel = pgm_read_byte(&cAttackPos[CountSeqAtt+11]);                              //11
  }
...
//  #endif          
  ...
  if(fSeqDelayEnd){
    ...
    if(CountSeqAttack == 0){
        ...
        NeutralStick[2] = 1;
        NeutralStick[3] = 1;
        HeadRotAngle1 = 0;                                     //Preset Head and Abdom angle
        HeadTiltAngle1 = 0; 
        HeadPanAngle1 = 0; 
        AbdomTiltAngle1 =0;
        AbdomPanAngle1 =0;
      ...
      if((digitalRead(EOM_p) == 0) && fSeqAttackPassed){      // if (EOM == 0) ->  CountSeqAttack = 22;
        CountSeqAttack = 22;
     #ifdef DEBUG
        ...
        if((digitalRead(EOM_p) == 0) && !fSeqAttackPassed){
...
void GetSeqPos(short CountSeqAtt){
  ...
  CountSeqAtt = (CountSeqAtt - 1) * 12;  
  byte iHeadRotAngle1 = pgm_read_byte(&cAttackPos[CountSeqAtt]);                          //0
  HeadRotAngle1 = (short)pgm_read_word(&cAttackHeadRotAngle1[iHeadRotAngle1]);

  byte iHeadTiltAngle1 = pgm_read_byte(&cAttackPos[CountSeqAtt+1]);                       //1
  HeadTiltAngle1 = (short)pgm_read_word(&cAttackHeadTiltAngle1[iHeadTiltAngle1]);

  byte iHeadPanAngle1 = pgm_read_byte(&cAttackPos[CountSeqAtt+2]);                        //2
  HeadPanAngle1 = (short)pgm_read_word(&cAttackHeadPanAngle1[iHeadPanAngle1]);

  byte iAbdomTiltAngle1 = pgm_read_byte(&cAttackPos[CountSeqAtt+3]);                      //3
  AbdomTiltAngle1 = (short)pgm_read_word(&cAttackAbdomenTiltAngle1[iAbdomTiltAngle1]);

  byte iAbdomPanAngle1 = pgm_read_byte(&cAttackPos[CountSeqAtt+4]);                       //4
  AbdomPanAngle1 = (short)pgm_read_word(&cAttackAbdomenPanAngle1[iAbdomPanAngle1]);

  if(!MandibleFSR_Activated) {
    POTCtrlPos = pgm_read_byte(&cAttackPos[CountSeqAtt+5]);                               //5
  }

  byte icBodyPozZ = pgm_read_byte(&cAttackPos[CountSeqAtt+7]);                            //6     Translate: 42 - forward, -42 - backwards
  g_InControlState.BodyPos.z = (short)pgm_read_word(&cBodyPozZ[icBodyPozZ]);

  byte icBodyRot1X = pgm_read_byte(&cAttackPos[CountSeqAtt+7]);                           //7     Attack: -127 forward; 127 - backwards
  g_InControlState.BodyRot1.x = (short)pgm_read_word(&cBodyRot1X[icBodyRot1X]);

  byte iSpeedControl = pgm_read_byte(&cAttackPos[CountSeqAtt+8]);                         //8 
  g_InControlState.SpeedControl = (short)pgm_read_word(&cSpeedControl[iSpeedControl]);           // 0 - max; 2000 - min

  BranchPWM_Eyes = pgm_read_byte(&cAttackPos[CountSeqAtt+9]);                             //9

  g_BodyYOffset = pgm_read_byte(&cAttackPos[CountSeqAtt+10]);                             //10   // 0 - min; MAX_BODY_Y=120
  ...
  if (g_fDebugOutput) {
    if (g_fDebugAttack) {
      ...
      DBGSerial.print(HeadRotAngle1, DEC);  
...
//-------------------------------------------------------------------------------
//PWM_Eyes(byte PWMIntervalMs, byte FromPWM, byte ToPWM, boolean EyesON)
//-------------------------------------------------------------------------------
// 
// 
//-------------------------------------------------------------------------------
boolean PWM_Eyes(byte PWMIntervalMs, byte* FromPWM, byte ToPWM, boolean EyesON){
#ifdef Eyes_p
